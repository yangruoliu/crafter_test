...
class SelectiveBlurWrapperWithRender(env_wrapper_v_a_star.SelectiveBlurWrapper):
...
def train_with_selective_blur():
    ...
    parser.add_argument('--target_obj_id', type=int, default=6, help='Target object ID (3=stone)')
    parser.add_argument('--target_obj_name', type=str, default='tree', help='Target object name')
    parser.add_argument('--blur_strength', type=int, default=6, help='Blur strength (odd number)')
    ...
    config = {
        ...
        "save_dir": os.path.join("RL_models_zdl", f"tree_with_blur_v2_a_star"),
        "model_name": f"tree_with_blur_v2_a_star",
    ...
    env = env_wrapper_v_a_star.InitWrapper(env, init_items=config["init_items"], init_num=config["init_num"])
    env = env_wrapper_v_a_star.WoodWrapper(env)
    ...
    reward_callback = RewardPlotCallback(
        save_interval=1000,
        plot_filename=f"training_rewards_tree_{timestamp}.png",
        verbose=1
    )
